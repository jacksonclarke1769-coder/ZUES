...
class VpcTrailManager:
    def __init__(self, *, account, signal_ts, side, qty, entry, init_stop_dist, trail_atr,
        ...
        self.pending = None                      # {"stop":.., "sent_at":.., "seq":..} in-flight replace
        self.stood_down = False                  # fail-safe latched: keep last resting stop, stop replacing
        self.exited = False
        self.replaces_sent = 0
        self.replaces_failed = 0

    # -- fail-safe timeout check on any in-flight replace ------------------------------------------
    def _check_timeout(self):
        if self.pending is None or self.stood_down:
            return
        if self.confirm_fn is not None:
            try:
                if self.confirm_fn(self.pending["stop"]):
                    self.resting_stop = self.pending["stop"]     # confirmed resting
                    self.pending = None
                    return
            except Exception:                                    # noqa: BLE001 — a broken confirm hook
                pass                                             # falls through to the timeout guard
        if self.clock_fn() - self.pending["sent_at"] > self.timeout_s:
            ...
            self.alert_fn("vpc_trail_timeout",
                          ...
                          detail=f"cancel-replace not confirmed in {self.timeout_s}s; keeping last "
                                 f"resting stop {self.resting_stop}; standing down further replaces")
            # keep resting_stop at the LAST CONFIRMED value (NOT the unconfirmed pending) — never naked
            self.pending = None
    ...
    def on_1m_bar(self, bar_id, low, high, close, atr_now, now_ts=None):
        ...
        # IDEMPOTENCY + RATE LIMIT: process each distinct 1m bar EXACTLY ONCE. A duplicate feed event
        # for the same bar_id must NOT re-step the canonical trail (double-ratchet would advance the
        # internal stop past the actually-working resting stop) and must NOT issue a second replace.
        if bar_id == self.last_bar_processed:
    ...
    def _issue_replace(self, new_stop, bar_id, now_ts):
        ...
        # NEVER-NAKED: place the NEW stop first; cancel the stale one only after it is accepted.
        ...
        self.send_fn(cancel_payload)               # best-effort cancel of the stale stop
        self.seq += 1
        self.replaces_sent += 1
        sent_at = now_ts if now_ts is not None else self.clock_fn()
        if self.confirm_fn is None:
            # NO-ACK model: a good send is optimistically treated as landed immediately; there is
            # nothing to confirm, so no in-flight window is tracked and the next bar may ratchet.
            self.resting_stop = new_stop
            self.pending = None
        else:
            # readback model: hold the replace in-flight (resting stop NOT yet advanced) until
            # confirm_fn sees it resting OR the timeout fires (fail-safe keeps the last resting).
            self.pending = {"stop": new_stop, "sent_at": sent_at, "seq": self.seq - 1}
